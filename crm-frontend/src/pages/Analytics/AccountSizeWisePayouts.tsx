...
import { accountSizePayoutAnalyticsService } from "@/services/accountSizePayoutAnalyticsService";

const AccountSizeWisePayouts = () => {
  const { data: analytics, isLoading, error } = useQuery({
    queryKey: ['account-size-wise-payouts'],
    queryFn: () => accountSizePayoutAnalyticsService.getAccountSizeWisePayouts(),
  });
  ...
  return (
    <div className="space-y-3 sm:space-y-6">
      <PageHeader
        title="Account Size Wise Payouts"
        subtitle="Revenue, payouts, and profit analysis by account size"
      />
      ...
      <Card>
        <CardHeader>
          <CardTitle>Account Size Breakdown</CardTitle>
          <CardDescription>
            Detailed analytics for each account size
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="overflow-x-auto">
            <Table>
              ...
              <TableBody>
                {analytics?.map((item) => (
                  <TableRow key={item.account_size}>
                    <TableCell className="font-medium">{formatCurrency(parseFloat(item.account_size))}</TableCell>
...
export default AccountSizeWisePayouts;
