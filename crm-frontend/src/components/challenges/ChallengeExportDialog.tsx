...
interface ChallengeExportDialogProps {
  isOpen: boolean;
  onClose: () => void;
}

export const ChallengeExportDialog: React.FC<ChallengeExportDialogProps> = ({
  isOpen,
  onClose,
}) => {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      if (challengeId && challengeId !== 'all') params.append('challenge_id', challengeId);
      if (status && status !== 'all') params.append('status', status);
      
      // Use quick date filter if selected, otherwise use date range
      if (quickDate) {
        params.append('quick_date', quickDate);
      } else {
      ...
      const url = `/admin/export-challenge-enrollments/${queryString ? `?${queryString}` : ''}`;
      ...
      link.download = `challenge_enrollments_${format(new Date(), 'yyyy-MM-dd')}.csv`;
      ...
      toast({
        title: "Success",
        description: "Challenge enrollments exported successfully",
      });
      ...
      toast({
        title: "Error",
        description: "Failed to export challenge enrollments",
        variant: "destructive",
      });
    } finally {
      ...
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Export Challenge Enrollments
          </DialogTitle>
          <DialogDescription>
            Configure export filters and download challenge enrollment data as CSV
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="space-y-2">
            ...
            <Select value={challengeId} onValueChange={setChallengeId} disabled={isLoadingChallenges}>
              <SelectTrigger id="challengeId">
                <SelectValue placeholder={isLoadingChallenges ? "Loading challenges..." : "Filter by challenge"} />
              </SelectTrigger>
